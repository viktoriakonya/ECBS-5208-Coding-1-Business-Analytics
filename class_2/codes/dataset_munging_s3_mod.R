...
# Create a factor variable for acc_type (use of factors will be clear in the next class) - for graphs
...
# df <- mutate( df , ratings = numeric( ratings ) )
...
gsub( "12" , "extra fun" , eg1 ) # what characters to find; what to replace; what value should be added
...
gsub("[^0-9\\.]","" , eg1) # negation (all non-numeric values)
...
sum( duplicated( df ) )
# Remove duplicates
df <- filter( df , !duplicated( df ) ) # sum up TRUE-s
...
hotel_vienna <- filter(df, city_actual == 'Vienna' 
                  & year == 2017 
                  & month == 11 
                  & weekend == 0
                  & stars %in% c(3,4) 
...
data_in <- "C:/Users/User/Documents/GitHub/ECBS-5208-Coding-1-Business-Analytics/class_2/data/hotels/"
data_out <- getwd()
write_csv( hotel_vienna , paste0( data_in,
                                  "/clean/hotel_vienna_restricted.csv"))
